...
\section{3-connectivity preservation under certified reductions}\label{app:3conn-preservation}

In Paper~1, $3$-connectivity is enforced by the replay checker as part of admissibility (see \S\ref{subsec:checker-semantics}). Accordingly, the main proof does not rely on a handwritten $3$-connectivity preservation argument.

The separator-lifting sketch below is included only to clarify the intended mechanism of the patch replacements; it is not used elsewhere in the correctness chain for Paper~1.

\begin{lemma}[Heuristic separator lifting (not used in Paper~1)]
Let $(G,\pi)\in\mathcal{Q}$ be $3$-connected, and let $(G',\pi')$ be obtained by applying a certified reduction inside a disk patch with boundary
attachments $B$.
If $G'$ has a $2$-vertex separator $\{a,b\}$, then $G$ has a $2$-vertex separator.
\end{lemma}

\begin{proof}[Proof sketch]
Outside the patch, $G$ and $G'$ are identical. The reduction only contracts/replaces vertices inside the disk while keeping the boundary attachments $B$
fixed (as vertices) and preserving the boundary cyclic order.

Take a $2$-vertex separator $\{a,b\}$ in $G'$ and consider two vertices $u,v$ in distinct components of $G'-\{a,b\}$.
If neither $a$ nor $b$ lies in the gadget, then the same separation holds in $G$ since the outside is unchanged and the patch replacement is confined.
If one or both lie in the gadget, expand the gadget back to the original patch.
Because the boundary attachments are unchanged and the patch is connected, any path in $G$ between the separated components would project to a path in $G'$
between the corresponding projected vertices, contradicting that $\{a,b\}$ separates in $G'$.
Thus a $2$-separator in $G'$ induces a $2$-separator in $G$, contradicting $3$-connectivity of $G$.
\end{proof}

This argument is standard for boundary-faithful disk replacements; in Paper~1, $3$-connectivity is verified mechanically by the replay checker after each step as part of admissibility.
...
\begin{itemize}
    ...
    \item \verb|local_types.jsonl|: \texttt{33DDAFC1B4E2ADE35BC399BAB78509069DE68564F91...}
    \item \verb|extendible_witnesses.jsonl|: \texttt{8CA386973B1060453A877B732420F5EBF1401A95...}
\end{itemize}
