...
can occur only below the base threshold $N_{\mathrm{base}}$ (and in the current verification run, no such obstructions were found up to $14$ vertices).
...
We call a certified occurrence \emph{admissible} (for the inductive proof) if applying the corresponding reduction produces a state $(G',\pi')\in\mathcal{Q}$.
In Section~\ref{sec:reductions} we prove that every certified occurrence in a state of $\mathcal{Q}$ is admissible; the implementation still
re-checks admissibility with an independent checker as a redundant audit.
\end{definition}

\begin{theorem}[Certified reductions preserve $\mathcal{Q}$]\label{thm:preserveQ}
Let $(G,\pi)\in\mathcal{Q}$ and let $R\in\mathcal{R}$ be any certified reduction rule.
If $R$ occurs in $(G,\pi)$ with a valid certificate, then the reduced state $(G',\pi'):=\mathsf{Reduce}_R(G,\pi)$ satisfies $(G',\pi')\in\mathcal{Q}$.
\end{theorem}

\begin{proof}
Planarity and the rotation system are preserved by the explicit rotation-system update built into each rule (Lemma~\ref{lem:rotation-update}),
so $(G',\pi')$ remains a plane embedding.
Cubicity and bipartiteness follow from the local degree bookkeeping and parity constraints enforced by the certificate: every deleted vertex is replaced
by the gadget vertices with the same number of incident darts, and the gadget is bipartite with the boundary attachments landing in opposite color classes.

Simplicity (no loops or parallel edges) is enforced by the distinctness constraints in the certificate (and is re-checked by the auditor for redundancy).
Finally, $3$-connectivity is preserved because each reduction is a disk replacement that is boundary-faithful: any $2$-vertex separator in $G'$
can be lifted to a $2$-vertex separator in $G$ by expanding the gadget back to the original patch and using the fact that the attachment boundary
remains unchanged. A detailed separator-lifting argument is given in Appendix~\ref{app:3conn-preservation}.
\end{proof}

\begin{theorem}[Certified completeness above the base threshold]\label{thm:certified-completeness}
Let $(G,\pi)\in\mathcal{Q}$ with $|V(G)|>N_{\mathrm{base}}$.
Then $(G,\pi)$ contains a certified occurrence of some rule $R\in\mathcal{R}$, and hence admits at least one admissible reduction step.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:fourfaces} there exists a facial $4$-cycle in $G$.
Apply Theorem~\ref{thm:completeness} to this face: one of the configurations $\mathcal{C}_2$, refined $\mathcal{C}_4$, or $\mathcal{C}_{\mathrm{pinch(ii)}}$
occurs topologically.

The rotation constraints appearing in Definitions~\ref{def:C2}--\ref{def:Cpinch} are not additional restrictions beyond the plane embedding: at a cubic
vertex the two possible cyclic orders are exactly the two orientations of the same embedding, and each definition allows both orders (after the
canonical relabeling of symmetric terminals). Therefore the topological occurrence canonically induces a valid certificate for the corresponding rule.
Admissibility then follows from Theorem~\ref{thm:preserveQ}.
\end{proof}

\paragraph{Remark (computed sanity checks).}
The scripts in the repository additionally enumerate rooted local types and search for obstruction witnesses up to a fixed cap as an
independent validation of the implementation; these computations are \emph{not} logically required for Theorem~\ref{thm:certified-completeness},
but help audit the pipeline and are reported in Appendix~\ref{app:repro-notes}.
...
We sketch the separator-lifting argument used in Theorem~\ref{thm:preserveQ}.
Each certified reduction replaces an embedded disk patch $P$ (the constant-size pattern neighborhood) by a constant-size gadget $P'$ with the same
attachment boundary (the same external attachment vertices and the same cyclic order of boundary darts).

\begin{lemma}[Separator lifting]
...
This argument is standard for boundary-faithful disk replacements; the auditor additionally checks $3$-connectivity after each step as a redundant
...
\begin{itemize}
    ...
    Runs \verb|python src/enumerate_local_types.py| and \verb|python tests/verify_completeness.py| to generate local types and confirm admissible reducibility (up to $N_{\max}=14$ in the verified run).
