...
We present a certified reduction framework (CRF) for this class $\mathcal{Q}$ and use it to construct a Hamiltonian cycle in every input graph.
The proof is organized around (i) an unavoidable local catalog derived from Euler's formula and bipartite parity (forcing facial $4$-cycles),
(ii) certified local reduction rules ($C_2$, $C_P=\mathrm{pinch}(ii)$, refined $C_4$) specified at the level of rotation systems,
and (iii) a finite lifting library that reconstructs a Hamiltonian cycle in the pre-reduction graph from a cycle in the reduced graph.
...
To minimize trusted code, each run emits a machine-readable certificate trace (boundary data, explicit terminal slot replacements, and gadget-rotation choices) that an independent checker replays,
...
Together, these components yield a reproducible, replay-checked proof object for each instance.
\end{abstract}
...
Graphs are finite, simple, undirected unless noted. An embedded plane graph is represented by a rotation system $\pi = \{\pi_v : v \in V\}$ where $\pi_v$ is a cyclic permutation of $N(v)$. All embeddings are assumed cellular. The class $\mathcal{Q}$ is precisely defined in Section~\ref{sec:introduction}.
...
The CRF combines (a) a short unavoidability argument (Euler + even face lengths) that guarantees a facial $4$-cycle and, locally around such a face, one of a finite catalog of reducible configurations;
(b) rotation-system--level reduction rules that replace a constant-size patch by a constant-size gadget while preserving membership in $\mathcal{Q}$;
and (c) a finite lifting library that reconstructs a Hamiltonian cycle through the replaced patch.
...
\subsection{Definitions: The Class \texorpdfstring{$\mathcal{Q}$}{Q}}
...
\begin{enumerate}
    ...
    \item \textbf{Planar}: $G$ has a cellular planar embedding via a rotation system $\pi$.
\end{enumerate}
\end{definition}
...
The IAM analyzes how global structures (Hamilton cycles) interact with local configurations. Key tools: terminal interfaces, interface types, path libraries, and a deterministic splicing criterion.
...
For a rotation system $(G,\pi)$, let $\alpha(v,u)=(u,v)$ be the arc-reversal map and let $\rho_\pi(v,u)=(v,\pi_v(u))$ be the induced dart-rotation permutation.
The \emph{face successor} is the permutation $\varphi = \rho_\pi \circ \alpha$ on $D(G)$. The \emph{faces} of the embedding are the orbits of $\varphi$ on $D(G)$.
\end{definition}
...
\begin{itemize}
    ...
    \item The cyclic ordering of terminals $T$ induced by the current rotation system (boundary order $\sigma$ up to reversal).
    \item Rotation slots (relative positions in $\pi_u$) for gadget re-insertion.
    \item A \textbf{flip bit} $\varepsilon\in\{0,1\}$ when needed to choose one of the two mirror disk embeddings of the gadget (for $\mathrm{pinch}(ii)$ this is the ``between'' choice).
\end{itemize}
\end{definition}
...
i.e.\ $v$ is a cut-vertex. Since $G$ is 3-connected, it has no cut-vertex, so no face boundary can repeat a vertex.
...
\begin{itemize}
    ...
    \item Each reduction strictly decreases the first coordinate $n(G)$ (hence strictly decreases $\mu$).
\end{itemize}
...
\]
The sum over $k\ge 8$ is non-positive, hence
...
\begin{theorem}[Topological unavoidability of the catalog]
\label{thm:completeness}
Every $G\in\mathcal Q$ contains a facial $4$-cycle whose radius-$1$ neighborhood realizes a \emph{topological} instance of at least one pattern from
$\{C_2,\ C_P,\ \text{refined }C_4\}$ (adjacent $4$-faces, a pinch(ii) identification, or an edge-isolated $4$-face).
In particular, $G$ contains a topological occurrence of one of these three configurations in the sense of \S\ref{subsec:topological-vs-certified}.
\end{theorem}
...
\]
in cyclic order.
Since $G$ is cubic, each $v_i$ has exactly one neighbor outside $F$; denote it by $u_i$.
Thus
\[
...
\medskip
\noindent\textbf{Case 1: $F$ shares an edge with another $4$-face.}
If some boundary edge $v_iv_{i+1}$ is incident (on its other side) to a facial $4$-cycle,
then $G$ contains two facial $4$-cycles sharing an edge, i.e.\ a \emph{topological} $C_2$ pattern.
As explained in \S\ref{subsec:topological-vs-certified}, the inductive step of Algorithm~\ref{alg:main} operates on \emph{certified} occurrences; the existence of a certified admissible step is discharged by the local-type completeness computation (Theorem~\ref{thm:certified-completeness}).

\medskip
\noindent\textbf{Case 2: $F$ is not adjacent (by an edge) to any other $4$-face.}
Then $F$ is edge-isolated in the sense of the catalog definition of refined $C_4$.
...
\smallskip
\noindent\emph{Proof.}
Along the $4$-cycle $F$ the bipartition alternates. Adjacent vertices $v_i$ and $v_{i+1}$
...
Thus $G-\{w,z\}$ disconnects $V(Q)$ from the rest of the graph (indeed it isolates $F$),
...
If $u_1,u_2,u_3,u_4$ are pairwise distinct, then by definition $F$ is a refined $C_4$
...
\smallskip
\noindent
Thus the local neighborhood around $F$ matches the definition of a $C_P$
...
We use the patch-interface notion from Definition~\ref{def:patch}:
...
\item \textbf{Certificate payload (including canonical boundary order).}
...
The certificate also records the boundary order $\sigma=(t_1,t_2,t_3,t_4)$ of terminals on the patch boundary (up to reversal), as read from the current embedding.

For $C_2$ we fix the \emph{canonical} terminal boundary order to be
\[
(u_1,u_4,u_5,u_6)
\quad\text{up to reversal},
\]
i.e.\ the terminals incident to the four boundary vertices $(a,d,e,f)$ in cyclic order around the patch boundary.
The checker normalizes the recorded $\sigma$ to this convention (modulo reversal) before applying the corresponding rotation listing and lifting tables.
\end{enumerate}
...
For each $i$, the certificate includes $\sigma(u_i)=(\ell_{u_i},v_i,r_{u_i})$ and the terminal boundary order $(u_1,u_2,u_3,u_4)$ (up to reversal) induced by the embedding around the face $Q$.
\end{enumerate}
...
reduction and lifting library are defined to accommodate such edges.
\end{definition}
...
\item terminal-slot data $\sigma(r),\sigma(s),\sigma(u_2),\sigma(u_4)$ and a flip bit $\varepsilon\in\{0,1\}$ encoding the disk-side choice.
\end{itemize}
...
\smallskip
\noindent\emph{Remark.} Since $w$ and $t$ are cubic, the local rotation at each is determined up to reversal; the certificate therefore only records the resulting flip bit $\varepsilon$ together with the boundary order of the underlying facial 4-cycle.
...
\item \textbf{Certificate payload.}
For each $t'\in T$, the certificate includes $\sigma(t')=(\ell_{t'},h(t'),r_{t'})$ and the terminal boundary order $\sigma=(t_1,t_2,t_3,t_4)$ (up to reversal), plus the flip bit $\varepsilon$.
\end{enumerate}
\end{definition}
...
\begin{itemize}
  \item a configuration type $\tau\in\{\mathrm{C2},\mathrm{CP},\mathrm{RefinedC4}\}$;
  \item the deleted vertex-set $V(H)$ (the configuration subgraph);
  \item the terminal set $T$ and its cyclic order $\sigma=(t_1,t_2,t_3,t_4)$
        as it appears on the boundary of a disk containing $H$ (up to reversal);
  \item the terminal map $h:T\to V(H)$ specifying, for each terminal $t$,
        which neighbor $h(t)$ is removed and must be replaced;
  \item a flip bit $\varepsilon\in\{0,1\}$ when the gadget has two mirror embeddings inside the disk (used for $\mathrm{CP}$ and allowed generically).
\end{itemize}
The checker replays a step \emph{only} from this information (plus the current $(G,\pi)$).
...
\]
The attachment map $h:T\to V(H)$ is part of the certificate and is unique by the catalog definitions.
...
\begin{enumerate}
  ...
  \item \textbf{Insert} the gadget vertices and gadget edges prescribed by the
        configuration type (Definitions~\ref{def:red-c2}--\ref{def:red-pinch}).
  \item \textbf{Rewire terminals.} For each terminal $t\in T$:
        add the edge $t g(t)$ where $g(t)$ is the gadget attachment vertex for $t$.
        Set
        \[
          \pi'_t := \textsc{ReplaceNeighbor}(\pi_t;\ h(t)\mapsto g(t)).
        \]
  \item \textbf{Set gadget rotations.} Assign $\pi'_v$ for each new gadget
        vertex $v$ using the boundary order $\sigma$ and flip bit $\varepsilon$ in the certificate
        (Appendix~\ref{app:rotation-listings} gives explicit canonical listings).
        All other $\pi'_u$ remain unchanged.
\end{enumerate}
\end{definition}
...
(ii) the terminal-slot replacement is exactly \textsc{ReplaceNeighbor};
...
%--------------------------------------------------------
\subsection{Reduction rule for \texorpdfstring{$\mathbf{C_2}$}{C2}}
\label{subsec:red-c2}

\begin{definition}[$C_2$ reduction]
\label{def:red-c2}
Let $\mathrm{cert}$ be a certified $C_2$ occurrence (Definition~\ref{def:C2-occ}) with deleted subgraph
$H_{C_2}=G[\{a,b,c,d,e,f\}]$ and terminals $T=\{u_1,u_4,u_5,u_6\}$.
The $C_2$ reduction deletes $V(H_{C_2})$ and inserts a \CtwoGadgetVertices-vertex gadget with new vertices $x,y$ and edges
\[
xy,\quad xu_1,\ xu_6,\ yu_4,\ yu_5.
\]
Thus $\Delta n=\CtwoDelta$.
The rotations at $x,y$ are set to realize the boundary order $\sigma$ recorded in the certificate (up to reversal); a canonical listing is given in Appendix~\ref{app:rot-c2}.
\end{definition}
...
\label{def:red-c4}
Let $\mathrm{cert}$ be a certified refined $C_4$ occurrence (Definition~\ref{def:refinedC4-occ}) with facial cycle
$Q=v_1v_2v_3v_4$ and terminals $T=\{u_1,u_2,u_3,u_4\}$, where $u_i$ is the unique neighbor of $v_i$ outside $Q$.
The refined $C_4$ reduction deletes $V(Q)$ and inserts a \RefinedCFourGadgetVertices-vertex gadget with new vertices $x,y$ and edges
\[
xy,\quad xu_1,\ xu_3,\ yu_2,\ yu_4.
\]
Thus $\Delta n=\RefinedCfourDelta$.
The rotations at $x,y$ are set to realize the boundary order recorded in the certificate (the cyclic order of terminals around the deleted 4-face); a canonical listing is given in Appendix~\ref{app:rot-refinedc4}.
\end{definition}

%--------------------------------------------------------
\subsection{Reduction rule for \texorpdfstring{$C_P$}{CPinch(ii)}}
\label{subsec:red-pinch}

\begin{definition}[$C_P$ reduction ($=\mathrm{pinch}(ii)$)]
\label{def:red-pinch}
Let $\mathrm{cert}$ be a certified $C_P$ occurrence (Definition~\ref{def:pinchii-occ}) with deleted patch
$H_{C_P}=G[\{v_1,v_2,v_3,v_4,w,t\}]$ and terminals $T=\{r,s,u_2,u_4\}$.
The $C_P$ reduction deletes $V(H_{C_P})$ and inserts a \PinchGadgetVertices-vertex gadget with new vertices $x,y$ and edges
\[
xy,\quad xr,\ xs,\ yu_2,\ yu_4.
\]
Thus $\Delta n=\PinchIIGadgetDelta$.
The flip bit $\varepsilon$ in the certificate selects one of the two mirror embeddings; the rotations at $x,y$ are set accordingly (Appendix~\ref{app:rot-pinch}).
\end{definition}
...
The cycles of $\varphi$ are the \emph{facial walks} of $(G,\pi)$.
...
a constant number of cyclic orders (at the boundary terminals and gadget vertices).
...
Form $\pi'$ from $\pi$ by applying $\mathrm{ReplaceNeighbor}(\pi_v;a_v\mapsto b_v)$ at each $v\in S$,
and leaving all other cyclic orders unchanged.
Then every $\varphi$-cycle of $(G,\pi)$ that avoids darts incident to $S$ remains a $\varphi'$-cycle of $(G,\pi')$.
\end{lemma}
\begin{proof}
If a dart $d$ is not incident to any vertex in $S$, then the rotation at its origin is unchanged, so $\rho_{\pi'}(d)=\rho_{\pi}(d)$ and hence $\varphi_{\pi'}(d)=\varphi_{\pi}(d)$.
Thus the orbit of $d$ under $\varphi_{\pi'}$ equals its orbit under $\varphi_{\pi}$.
\end{proof}
...
\begin{enumerate}
  ...
  \item $\pi'$ is obtained from $\pi$ by (i) deleting $H$ and its incident darts,
        (ii) inserting $K$ and its darts with rotations on $V(K)\setminus T$,
        and (iii) at each $t\in T$ applying $\mathrm{ReplaceNeighbor}(\pi_t;h(t)\mapsto g(t))$.
\end{enumerate}
Then $(G',\pi')$ is a cellular plane embedding of the reduced graph $G'$.
...
is glued consistently. The face-successor permutation model then yields a cellular embedding on the sphere.
\end{proof}

% ============================================================
% Generic planarity lemma for 2-vertex rotation-system surgery
% ============================================================
...
\label{def:2v-gadget}
Let $u_1,u_2,u_3,u_4$ be four (not necessarily consecutive) boundary vertices on the boundary of a topological disk.
...
Let $H$ be a disk patch with boundary terminals $(u_1,u_2,u_3,u_4)$ in the cyclic order induced by the embedding,
and form $G'$ from $G$ by deleting $H$ and inserting $K^{(2)}_{\sigma,\varepsilon}$ inside the same disk, splicing attachment edges at the boundary terminals via \textsc{ReplaceNeighbor}.
Then the resulting rotation system $\pi'$ is planar and cellular.
\end{lemma}

\begin{proof}
This is a direct instance of Lemma~\ref{lem:embedding-patch} with $K=K^{(2)}_{\sigma,\varepsilon}$:
the gadget is embedded in the disk with boundary order matching the terminals' cyclic order (up to reversal),
and terminal rotations are changed only by slot splicing.
\end{proof}
...
\label{cor:planar-C2}
For a certified $C_2$ reduction step, the deleted domino subgraph forms a disk patch whose boundary terminals are the four terminals in the boundary order recorded by the certificate trace (normalized to the canonical order of Definition~\ref{def:C2-occ}, up to reversal).
The reduced graph is obtained by inserting the corresponding \CtwoGadgetVertices-vertex gadget inside the same disk and splicing at terminals by \textsc{ReplaceNeighbor}.
Therefore the resulting rotation system is planar by Lemma~\ref{lem:generic-2v-planarity}.
\end{corollary}

\begin{corollary}[Planarity of the $\mathrm{pinch}(ii)$ reduction]
\label{cor:planar-pinchii}
For a certified $\mathrm{pinch}(ii)$ step, the deleted patch forms a disk patch whose boundary terminals are $(r,s,u_2,u_4)$ in the boundary order recorded by the certificate trace (up to reversal), and the reduced graph is obtained by inserting the corresponding \PinchGadgetVertices-vertex gadget with the recorded flip bit.
Therefore the resulting rotation system is planar by Lemma~\ref{lem:generic-2v-planarity}.
\end{corollary}

\begin{corollary}[Planarity of the refined $C_4$ reduction]
\label{cor:planar-refinedC4}
For a certified refined $C_4$ step, the deleted facial 4-cycle forms a disk patch whose boundary terminals are $(u_1,u_2,u_3,u_4)$ in the cyclic order induced by the embedding around that face (up to reversal), and the reduced graph is obtained by inserting the \RefinedCFourGadgetVertices-vertex gadget and splicing at terminals by \textsc{ReplaceNeighbor}.
Therefore the resulting rotation system is planar by Lemma~\ref{lem:generic-2v-planarity}.
\end{corollary}

\begin{remark}[What the trace must record for planarity checking]
For each reduction step, the certificate trace must include:
(i) the terminal list in boundary order $\sigma$ (as read from the current embedding, and normalized where a canonical $\sigma$ is stipulated),
(ii) the attachment map $h$ (which neighbor is being replaced at each terminal),
(iii) the gadget type and the flip bit when applicable, and
(iv) the induced terminal rotation edits (\textsc{ReplaceNeighbor} slot replacement).
Given this, the checker can reconstruct $\pi'$ and verify planarity by enumerating $\varphi'$-cycles and checking Euler's formula.
\end{remark}
...
Let $G$ be bipartite with fixed $2$-coloring $(A,B)$.
Each reduction deletes vertices and inserts a gadget with two new vertices $x,y$ joined by an edge.
Since every terminal has exactly one neighbor inside the deleted patch, each terminal's color is already fixed, and each new attachment edge $t g(t)$ must connect opposite colors.
Thus the coloring extends uniquely by forcing $x$ and $y$ to be the colors opposite to their attached terminals.
Concretely, in each rule:
\begin{itemize}
\item Refined $C_4$: $x$ is opposite to $u_1$ (and hence also opposite to $u_3$), and $y$ is opposite to $u_2$ (and hence also opposite to $u_4$); $xy$ is then automatically bichromatic.
\item $C_2$: $x$ is opposite to $u_1$ (and $u_6$), and $y$ is opposite to $u_4$ (and $u_5$).
\item $C_P$: $x$ is opposite to $r$ (and $s$), and $y$ is opposite to $u_2$ (and $u_4$).
\end{itemize}
The checker enforces bipartiteness directly by a BFS parity test after each replayed step.

\subsection{Cubicity Preservation}
\label{subsec:cubicity}

Each reduction rewires each terminal $t\in T$ by deleting exactly one neighbor in the patch and adding exactly one neighbor in the gadget, preserving $\deg(t)=3$.
Each newly inserted gadget vertex $x$ or $y$ has exactly three incident edges by construction (two to terminals and one internal $xy$).
No other vertices change degree. Hence all degrees remain $3$.
...
During replay, the checker recomputes $3$-connectivity of each replayed abstract graph $G'$.
...
Because the certified configuration language is local, certified occurrences can be detected rapidly,
but admissibility depends on global invariants (most importantly $3$-connectivity).
For the computer-assisted proof we therefore execute only those certified steps whose replay is accepted by the checker;
this guarantees membership in $\mathcal{Q}$ for every intermediate state (Theorem~\ref{thm:preserveQ}).

The remaining issue is completeness in this \emph{admissible} sense:
every non-base state must contain \emph{some} admissible configuration occurrence.
We do \emph{not} require that every certified occurrence be admissible.
Admissible-step completeness is supplied by the finite local-type theorem
(Theorem~\ref{thm:certified-completeness}), which is verified by exhaustive enumeration of all radius-$3$ neighborhoods
around a facial $4$-cycle and the production of a replay-checkable witness step for each enumerated type.

\section{Interface Analysis for Hamiltonian Cycles}
\label{sec:interface-analysis}

For each certified reduction, we must analyze how a Hamiltonian cycle in the reduced graph intersects the gadget.
This analysis produces a finite lifting library that enables deterministic reconstruction of Hamiltonian cycles in the original graph.
...
constant-size gadget $K$ attached to a fixed terminal set $T=\{t_1,t_2,t_3,t_4\}$.
...
(a,b,c,d)=(r,s,u_2,u_4).
\]
(The flip bit $\varepsilon$ affects the embedding inside the patch but not the abstract interface type.)
...
\begin{itemize}
    ...
    \item Terminal degrees remain 2: if $C'$ used the gadget edge at $t$, we replace it by the unique original boundary edge $t$-$h(t)$; if $C'$ did not, we do not introduce $t$-$h(t)$.
    \item Every vertex in $H$ has degree 2 in $\mathcal{P}$, hence in the resulting cycle.
    \item Replacing a subpath on a single cycle by another path with the same endpoints preserves being a single cycle; doing this on two disjoint subpaths preserves being a single cycle as well.
\end{itemize}
...
A lifting table is correct if for every row, the specified path system $\mathcal{P}$ satisfies the requirements of Lemma~\ref{lem:splicing}. Specifically, we verify:
...
\subsection{Lifting for the \texorpdfstring{$C_2$}{C2} Reduction}
...
\textbf{Prereqs:} Lemma~\ref{lem:splicing}.
For each $C_2$ interface type in Table~\ref{tab:lifting-c2}, the listed path system
satisfies Lemma~\ref{lem:splicing}.
\end{lemma}

\begin{proof}
Apply Lemma~\ref{lem:table-correctness}; this is verified exhaustively by the lifting-table verifier (Appendix~\ref{app:reproduction}, \verb|src/verify_gadgets.py|), which checks that each row in Table~\ref{tab:lifting-c2} connects the correct terminals and covers the vertices $\{a,b,c,d,e,f\}$ exactly once.
\end{proof}

\subsection{Lifting for the Refined \texorpdfstring{$C_4$}{C4} Reduction}
...
In refined $C_4$, the parallel interface type $\mathsf{P}$ cannot occur.
\end{lemma}

\begin{proof}
A parallel interface would require two vertex-disjoint paths through the deleted patch whose endpoints are $(u_1,u_3)$ and $(u_2,u_4)$, and whose internal vertices are exactly $\{v_1,v_2,v_3,v_4\}$.
But any $u_1$--$u_3$ path through the patch must enter at $v_1$ and exit at $v_3$, and hence must use at least one of $v_2$ or $v_4$ as an internal vertex.
The remaining two vertices are insufficient to form a vertex-disjoint $u_2$--$u_4$ path entering at $v_2$ and exiting at $v_4$ while keeping all degrees $2$ in the union.
Thus $\mathsf{P}$ is impossible.
\end{proof}
...
system satisfies Lemma~\ref{lem:splicing}.
\end{lemma}

\begin{proof}
This is verified exhaustively by the lifting-table verifier (Appendix~\ref{app:reproduction}, \verb|src/verify_gadgets.py|), which checks that each path in Table~\ref{tab:lifting-c4} connects the correct terminals and covers $V(H)$ exactly once. The result follows from Lemma~\ref{lem:table-correctness}. Parallel interfaces are excluded by Lemma~\ref{lem:no-parallel-C4}.
\end{proof}

\subsection{Lifting for the \texorpdfstring{$C_P$}{CP} Reduction}
...
path system satisfies Lemma~\ref{lem:splicing}.
\end{lemma}
...
\label{lem:projection-c2}
\prereq{Bipartite parity on boundary edges}
Let $G$ be a graph in $\mathcal{Q}$ containing a certified $C_2$ configuration. Let $H$ be a Hamiltonian cycle in $G$. Then the intersection of $H$ with the configuration boundary edges $B = \{u_1a, u_4d, u_5e, u_6f\}$ has size $2$ or $4$, and the induced terminal pairing is one of the five interface types in Table~\ref{tab:lifting-c2}.
\end{lemma}

\begin{proof}
Since $H$ is a cycle, $|H\cap B|$ is even. It cannot be $0$ because then $H$ would avoid the patch entirely, forcing a disconnection in the induced subgraph. Thus $|H\cap B|\in\{2,4\}$.
If $|H\cap B|=4$, the patch contributes two disjoint subpaths, giving the parallel type.
If $|H\cap B|=2$, the patch contributes a single subpath joining the two chosen terminals, giving one of the four cross types.
\end{proof}

\begin{lemma}[Projection Lemma for Refined $C_4$]
\label{lem:proj-C4}
\prereq{Bipartite parity on boundary edges}
Let $G$ be a graph in $\mathcal{Q}$ containing a certified refined $C_4$ configuration. Let $H$ be a Hamiltonian cycle in $G$. Then the induced interface is one of the four cross interface types described in Table~\ref{tab:lifting-c4}.
\end{lemma}

\begin{proof}
Consider the boundary edges $B = \{u_1v_1, u_2v_2, u_3v_3, u_4v_4\}$. Since $H$ is a cycle, $|H \cap B|$ is even and nonzero.
If $|H\cap B|=4$, this would force a parallel interface, which is impossible by Lemma~\ref{lem:no-parallel-C4}. Hence $|H\cap B|=2$ and the interface is a single cross path, yielding exactly the four cross types.
\end{proof}

\begin{lemma}[Projection Lemma for $C_P$]
\label{lem:projection-pinch}
\prereq{Bipartite parity on boundary edges}
Let $G$ be a graph in $\mathcal{Q}$ containing a certified $C_P$ configuration. Let $H$ be a Hamiltonian cycle in $G$. Then the induced interface is one of the five interface types described in Table~\ref{tab:lifting-pinch}.
\end{lemma}

\begin{proof}
Analyze the boundary edges $B = \{rt, st, u_2v_2, u_4v_4\}$. Since $H$ is a cycle, $|H \cap B|$ is even and nonzero, hence $|H\cap B|\in\{2,4\}$. The cases $|H\cap B|=4$ and $|H\cap B|=2$ yield the parallel type and the four cross types, respectively.
\end{proof}
...
For each certified reduction in the catalog:
\begin{enumerate}
...
\begin{proof}
\textbf{Lifting}: Follows from Lemma~\ref{lem:splicing} and the lifting tables (Lemmas~\ref{lem:lifting-c2}, \ref{lem:lifting-c4}, \ref{lem:lifting-pinch}).

\textbf{Projection}: Given a Hamiltonian cycle $H$ in $G$, apply the corresponding Projection Lemma (Lemmas~\ref{lem:projection-c2}, \ref{lem:proj-C4}, \ref{lem:projection-pinch}) to identify the interface type. Replace the intersection of $H$ with the deleted patch by the corresponding gadget edges. This yields a Hamiltonian cycle in $G'$ by the reverse splicing argument.
\end{proof}
...
\label{thm:lifting}
\prereq{Lifting Library, Lemma~\ref{lem:splicing}}
For each certified reduction, given a Hamiltonian cycle $H'$ in the reduced graph $G'$,
the lifting procedure produces a Hamiltonian cycle $H$ in the pre-reduction graph $G$ in $O(1)$ time.
\end{theorem}

\begin{proof}
Each gadget has constant size. By Lemma~\ref{lem:interfaces-2vertex}, there are exactly five possible abstract interface types for any 2-vertex gadget; for refined $C_4$, Lemma~\ref{lem:no-parallel-C4} excludes the parallel type, leaving four.
The lifting procedure identifies the interface type by checking which gadget edges are used in $H'$ (constant time), retrieves the corresponding path system from the library (constant time via table lookup), and replaces the gadget's contribution with this path system.
By Lemma~\ref{lem:splicing}, this yields a Hamiltonian cycle in $G$.
\end{proof}

\section{Base Cases and Reproducible Verification}
\label{sec:base-cases}

\begin{definition}[Base threshold $N_{\mathrm{base}}$]\label{def:Nbase}
Fix an integer $N_{\mathrm{base}}\ge 1$. Our reduction algorithm is allowed to stop once the current graph has at most $N_{\mathrm{base}}$ vertices, at which point we invoke an exact base-case Hamiltonicity solver that returns an explicit Hamilton cycle (and a checker-verifiable certificate of correctness for that cycle).
In the verified implementation used for this paper we take $N_{\mathrm{base}}=14$.
\end{definition}
...
\subsubsection{Topological vs certified occurrences}
\label{subsec:topological-vs-certified}

Theorem~\ref{thm:completeness} is a purely combinatorial \emph{unavoidability} statement: it guarantees the existence of a \emph{topological} pattern around some facial $4$-cycle (adjacent $4$-faces, a pinch(ii) identification, or an edge-isolated $4$-face).
A \emph{topological occurrence} records only the embedding-local incidence information forced by the case analysis and does \emph{not} include certification side-conditions such as terminal distinctness, forbidden identifications, or terminal-slot data.

By contrast, the reduction rules in \S\ref{sec:catalog} are triggered only by \emph{certified occurrences}, which include (i) the required side-conditions, and (ii) the explicit rotation-system interface data needed for replay checking and deterministic lifting.
Algorithm~\ref{alg:main} and the replay checker therefore operate exclusively on certified occurrences.

In the present (computer-assisted) version, the bridge from topological unavoidability to certified reducibility is discharged computationally:
Theorem~\ref{thm:certified-completeness} enumerates all radius-$3$ rooted local types around a facial $4$-cycle consistent with $\mathcal{Q}$ and proves that every such type contains at least one admissible \emph{certified} configuration.


The unavoidability theorem (Theorem~\ref{thm:completeness}) guarantees that every $(G,\pi)\in\mathcal{Q}$ contains some facial $4$-cycle exhibiting a \emph{topological} instance of the catalog patterns.
For the inductive step we require a \emph{certified} occurrence (in the sense of \S\ref{subsec:topological-vs-certified}) that is admissible, i.e. its reduction preserves membership in $\mathcal{Q}$ and admits deterministic lifting.

\begin{definition}[Certified occurrence and admissibility]\label{def:cert}
Let $\mathcal{R}$ denote the set of certified reduction rules.
A \emph{certificate} for a rule $R\in\mathcal{R}$ consists of:
(i) an embedding-respecting map of the constant-size pattern of $R$ into $(G,\pi)$,
and (ii) local side-conditions (distinctness of terminals, forbidden identifications, and rotation constraints where relevant).
An occurrence is \emph{certified} if these local checks pass.

We call a certified occurrence \emph{admissible} if, after applying the deterministic
rotation-system surgery of Section~\ref{sec:reductions}, the replay checker accepts the post-state.
Equivalently, letting $(G',\pi'):=\textsc{Reduce}(G,\pi;\mathrm{cert})$ (Definition~\ref{def:reduce-op}),
admissibility means that $(G',\pi')$ satisfies all checker invariants:
simplicity, cubicity, bipartiteness, planarity-from-rotation (Euler characteristic via $\varphi$-orbits),
and $3$-connectivity.
\end{definition}

\begin{theorem}[Certified reductions preserve $\mathcal{Q}$ (checker-enforced)]\label{thm:preserveQ}
Let $(G,\pi)\in\mathcal{Q}$ and let $\mathrm{cert}$ be a certified configuration occurrence
(Definition~\ref{def:cert}).
If $\mathrm{cert}$ is admissible, then the reduced state $(G',\pi'):=\textsc{Reduce}(G,\pi;\mathrm{cert})$
satisfies $(G',\pi')\in\mathcal{Q}$.
Moreover, Hamiltonicity is preserved bidirectionally by Theorem~\ref{thm:bidirectional-hamiltonicity}.
\end{theorem}

\begin{proof}
By definition of admissibility, the checker verifies that $G'$ is simple, cubic, bipartite, and $3$-connected,
and that $\pi'$ is a planar cellular rotation system for $G'$ (via face traversal and Euler characteristic).
Hence $(G',\pi')\in\mathcal{Q}$.

Hamiltonicity preservation is purely local and follows from Theorem~\ref{thm:bidirectional-hamiltonicity},
independently of how admissibility is certified.
\end{proof}

\subsubsection*{Computational admissible completeness above the base threshold}

\begin{definition}[Radius-$3$ rooted local type at a $4$-face]\label{def:local-type}
Let $(G,\pi)\in\mathcal{Q}$ and let $Q$ be a facial $4$-cycle.
The \emph{radius-$3$ rooted local type} of $(G,\pi)$ at $Q$ is the embedded induced subgraph
on all vertices at graph distance at most $3$ from $V(Q)$, together with:
(i) the rotation system inherited from $\pi$, and
(ii) the marking of $Q$ as the root face.
Two rooted local types are isomorphic if there is a graph isomorphism that maps $Q$ to $Q$
(as an ordered facial $4$-cycle up to reversal) and preserves all cyclic orders.
\end{definition}

\begin{theorem}[Admissible-step completeness above the base threshold (computer-checked)]\label{thm:certified-completeness}
There exists a finite set $\mathcal{T}$ of rooted local types (Definition~\ref{def:local-type}) such that:
\begin{enumerate}
  \item Every $(G,\pi)\in\mathcal{Q}$ and every facial $4$-cycle $Q$ in it has rooted local type in $\mathcal{T}$.
  \item For every type $\tau\in\mathcal{T}$, at least one certified configuration occurrence
        from $\{C_2,C_P,\text{refined }C_4\}$ is admissible, and a corresponding witness certificate is produced.
\end{enumerate}
Consequently, if $(G,\pi)\in\mathcal{Q}$ satisfies $|V(G)|>N_{\mathrm{base}}$, then $(G,\pi)$ admits at least one admissible reduction step.
\end{theorem}

\begin{proof}
This theorem is established by finite computation.
The script \texttt{src/enumerate\_local\_types.py} enumerates all rooted radius-$3$ local types compatible with
the defining constraints of $\mathcal{Q}$ (cubic, bipartite, planar rotation system, and a rooted facial $4$-cycle).
The script \texttt{tests/verify\_completeness.py} then searches each enumerated type for a certified occurrence of
one of the catalog configurations and a certificate whose replayed reduction is accepted by the independent checker
(\S\ref{subsec:checker-semantics}). The resulting witness logs (and audit hashes) are recorded in Appendix~\ref{app:hashes}.

For the consequence, take any facial $4$-cycle $Q$ (exists by Lemma~\ref{lem:existence-f4}) and consider its rooted local type.
By (1) it is some $\tau\in\mathcal{T}$; by (2) $\tau$ contains an admissible certified occurrence, which yields an admissible reduction step in $(G,\pi)$.
\end{proof}

\paragraph{Remark (computational vs.\ fully analytic variants).}
For the present paper (computer-assisted but fully rigorous), Theorem~\ref{thm:certified-completeness} is part of the trusted proof pipeline:
it is a finite, replay-checkable claim about bounded local neighborhoods.
In a future purely mathematical version, one would replace this step by a fully analytic admissibility argument (and in particular a non-computational
$3$-connectivity preservation proof).

\subsection{Computational base verification}

\begin{theorem}[Base-case verification (computational)]
\label{thm:base-verified}
Fix $N_{\mathrm{base}}$ as in Definition~\ref{def:Nbase}. The base-case pipeline produces an artifact file
\verb|basecases.jsonl| consisting of a complete list of isomorphism types of $(G,\pi)\in\mathcal{Q}$ with
$|V(G)|\le N_{\mathrm{base}}$, together with an explicit Hamilton cycle certificate for each.
The script \verb|tests/verify_base_cases.py| replay-verifies every line of this file by checking:
(i) $(G,\pi)\in\mathcal{Q}$, and
(ii) the listed cycle is a Hamilton cycle of $G$.
In particular, for $N_{\mathrm{base}}=14$ this yields verified certificates for all base cases recorded in
\verb|basecases.jsonl|.
\end{theorem}

\begin{proof}
This is a finite, exhaustive computation over the published base-case list, with each claimed cycle verified
directly against the adjacency data of $G$.
\end{proof}
...
\EndIf
\State $C \gets \textsc{FindAdmissibleConfig}(G)$ \Comment{choose a certified occurrence; the postcheck is a redundant audit}
...
If $|V(G)|\le N_{\mathrm{base}}$, the algorithm returns a Hamiltonian cycle recorded in the base-case artifact and revalidated by the checker (Theorem~\ref{thm:base-verified}).

Otherwise, $|V(G)|>N_{\mathrm{base}}$.
By the certified-step completeness theorem (Theorem~\ref{thm:certified-completeness}), $G$ contains a certified configuration
$C\in\{C_2,C_P,\text{refined }C_4\}$ for which an admissible reduction step $G\to G'$ exists.
Property preservation implies $(G',\pi')\in\mathcal{Q}$ (Theorem~\ref{thm:preserveQ}), and the measure strictly decreases,
$\mu(G')<_{\mathrm{lex}}\mu(G)$ (Lemma~\ref{lem:measure-decrease}).
By the induction hypothesis, the recursive call returns a Hamiltonian cycle in $G'$.
Finally, the lifting theorem (Theorem~\ref{thm:lifting}, together with the bidirectional Hamiltonicity preservation statements in Theorem~\ref{thm:bidirectional-hamiltonicity})
lifts this cycle to a Hamiltonian cycle in $G$.
\end{proof}
...
Recall the lexicographic measure $\mu(G) \in \mathbb{N}^5$ from Section~\ref{sec:measure}.

\begin{lemma}[Well-foundedness of $(\mathbb{N}^5, <_{\text{lex}})$]
\label{lem:well-founded}
...
Restrict to the tail $i \ge N$. Then lex decreases must occur in the remaining coordinates, yielding an infinite descending chain in $\mathbb{N}^4$ under lex order. Repeating this argument coordinate-by-coordinate eventually yields an infinite descending chain in $\mathbb{N}$, which is impossible.
\end{proof}

\begin{lemma}[Strict Measure Decrease]
\label{lem:measure-decrease}
For any admissible certified reduction step $G \to G'$ using $C \in \{C_2, C_P, \text{refined } C_4\}$,
\[
\mu(G') <_{\text{lex}} \mu(G).
\]
\end{lemma}
...
\begin{proof}
Each reduction strictly decreases the vertex count $n(G)$ by either $2$ or $4$:
\[
\Delta n(\text{refined } C_4)=\RefinedCfourDelta=-2,\qquad
\Delta n(C_2)=\CtwoDelta=-4,\qquad
\Delta n(C_P)=\PinchIIGadgetDelta=-4.
\]
Thus $n(G')<n(G)$, so the first coordinate of $\mu$ decreases, hence $\mu(G')<_{\text{lex}}\mu(G)$.
\end{proof}

\begin{theorem}[Termination]
\label{thm:termination}
\prereq{Lemmas~\ref{lem:measure-decrease} and~\ref{lem:well-founded}}
There is no infinite sequence of admissible certified reductions
\[
...
\begin{proof}
By Lemma~\ref{lem:measure-decrease}, $\mu(G_{i+1}) <_{\text{lex}} \mu(G_i)$ for every step. By Lemma~\ref{lem:well-founded}, such an infinite strictly descending chain is impossible.
\end{proof}
...
\begin{itemize}
    \item \texttt{origin[h]} $\in V$,
    \item \texttt{twin[h]} $=\bar h$,
    \item \texttt{rotNext[h]} (next half-edge in the cyclic order around \texttt{origin[h]}),
    \item \texttt{rotPrev[h]},
    \item \texttt{face[h]} (face-id of the face to the left of $h$),
...
\]
Thus face boundaries are exactly the $\varphi$-orbits.

\paragraph{Vertices and faces.}
Each vertex stores one incident half-edge pointer \texttt{inc[v]} and an \texttt{alive[v]} bit.
Each face record stores \texttt{rep[f]}, \texttt{len[f]}, \texttt{alive[f]} and a version counter \texttt{ver[f]}.

\paragraph{Building the DCEL from a rotation system.}
Given the input rotation system $\pi=\{\pi_v\}$, we create one half-edge for each ordered pair $(v,u)$ with $u\in N(v)$, set \texttt{origin}, and link
\texttt{rotNext/rotPrev} according to $\pi_v$.
Twins are found by a single pass using a dictionary keyed by the ordered pair $(v,u)$.
Finally, we compute all faces by traversing $\varphi$-orbits.
All of this takes $O(n)$ time and $O(n)$ memory.
...
Adjacency to another 4-face across edge $v_iv_{i+1}$ is read as \texttt{len[ face[twin(h\_i)] ]}.
Pinch equalities $u_1=u_3$ or $u_2=u_4$ are direct pointer comparisons.
Rotation constraints are checked by inspecting the cyclic order of the three half-edges at $w$ or $t$.
...
Consequently, only faces incident to the boundary terminals and gadget half-edges can change.

\paragraph{Damage set.}
Let $B$ be the set of boundary terminals (at most 4) and let $V_K=\{x,y\}$ be the new gadget vertices.
Define the damage half-edge set
\[
...
For each half-edge $h\in D$ that is alive, we traverse its $\varphi$-orbit to recover the face boundary and its length,
...
Rotation splices at terminals change only constant many \texttt{rotNext/rotPrev} pointers.
...
Each pop performs $O(1)$ work, so total pop work is $O(\#\text{pushes})$.
\end{proof}
...
\end{theorem}
\begin{proof}
Initialization builds the DCEL and enumerates faces in $O(n)$ time. Each reduction step performs $O(1)$ updates by Lemma~\ref{lem:update-constant}, and queue maintenance is amortized $O(1)$ by Lemma~\ref{lem:queue-amortized}. Each reduction decreases $n$ by a positive constant, so there are $O(n)$ reductions. Hence total time is $O(n)$; memory is $O(n)$ by Lemma~\ref{lem:space}.
\end{proof}
...
enumeration on the finite gadget graphs. This is implemented by \texttt{src/verify\_gadgets.py}.
...
\end{enumerate}
Thus the lifting library is validated as both complete and correct.
...
The checker uses the standard face permutation $\varphi = \rho_\pi \circ \alpha$ (Section~\ref{subsec:definitions-model}). Let $F$ denote the number of $\varphi$-orbits (faces).
...
\item \textbf{Planarity-from-rotation.} Enumerate faces via $\varphi$ and check $|V|-|E|+F=2$.
\item \textbf{3-connectivity (mandatory).} The checker runs a 3-connectivity test on $G_i$ after each replayed step.
\end{enumerate}
...
and (iii) updating rotations at a constant set of vertices. The checker treats all terminal rotation edits as required to equal \textsc{ReplaceNeighbor} (Definition~\ref{def:replace-neighbor}), and compares cyclic orders up to cyclic shift at each vertex.
...
\item (optional) a hash of a canonical serialization of $(G_0,\pi_0)$.
\end{itemize}

\paragraph{Each step $s_i$.}
Required fields include: configuration type, a local embedding witness locating the configuration, boundary order $\sigma$ (up to reversal), flip bit $\varepsilon$ (when applicable), and the terminal rewires/slot data.

\paragraph{Replay semantics (deterministic).}
Given $S_{i-1}=(G_{i-1},\pi_{i-1})$ and a step $s_i$, the checker:
\begin{enumerate}
\item Validates schema and vertex/edge consistency.
\item Verifies preconditions (configuration subgraph and facial $4$-cycle witness).
\item Applies the abstract graph update.
\item Applies the rotation update, enforcing \textsc{ReplaceNeighbor} at terminals and fixed gadget rotations.
\item Runs the postcondition invariants of \S\ref{subsubsec:checker-invariants}.
\item Checks $\Delta n$ matches the expected rule delta (and optionally recomputes full $\mu$).
\end{enumerate}

\subsubsection{Cycle reconstruction and lifting verification (end-to-end check)}
\label{subsubsec:checker-cycle-lifting}

The trace ends with a base-case Hamiltonian cycle witness in the final reduced graph $G_m$.
The checker then lifts backwards, step-by-step, using interface decoding and the verified lifting tables, and verifies at each stage that the lifted object is a Hamiltonian cycle of the corresponding pre-image graph.

\subsection{Reproducibility}
All scripts and tests are included in the public repository; Appendix~\ref{app:reproduction} lists one-command reproduction targets and expected artifacts.

\section{Related Work}
\label{sec:related}

Our approach relates to:
\begin{itemize}
    \item \textbf{Certified Reductions}: certificate-driven frameworks for hereditary classes and proof replay.
    \item \textbf{Discharging}: typical methodology for unavoidable set proofs in planar graph theory.
    \item \textbf{Partial Results on Barnette's Conjecture}: results for restricted face-size classes (e.g.\ Goodey-type constraints).
\end{itemize}

\section{Conclusion}
\label{sec:conclusion}

We have presented a constructive, replay-checkable certified reduction framework for $\mathcal{Q}$.
The unavoidable set $\{C_2,\text{refined }C_4,C_P\}$, together with certified reductions and a finite lifting library, yields a Hamiltonian cycle construction whose correctness is validated by an independent checker and supported by exhaustive base-case verification up to $N_{\mathrm{base}}=14$.

\section*{Data Availability}
All code, scripts, and verification artifacts are available at:
\url{https://github.com/EnLilaSko/Barnettes-Conjecture} \\
Verified release: commit \texttt{7ef267f675136003180a38877a5615bf5e047c1ee} \\
...
Appendix~\ref{app:rotation-listings} lists explicit canonical gadget rotations.
In actual traces, the boundary order $\sigma$ is read from the current embedding (up to reversal), and the checker enforces that the inserted gadget rotations realize that boundary order.

\subsection{Conventions}
\label{app:rot-conventions}

All graphs are embedded as combinatorial maps (rotation systems). For a cubic embedded graph we write $\pi(v)=(a,b,c)$ to denote the cyclic order of neighbors at $v$ (up to cyclic shift). All rotations at vertices not mentioned explicitly are unchanged.

\subsection{\texorpdfstring{$C_2$}{C2} --- \texorpdfstring{\CtwoGadgetVertices}{2}-vertex gadget}
\label{app:rot-c2}

\noindent\textbf{Vertices added:} $x,y$. \\
\noindent\textbf{Edges added:} $xy, xu_1, xu_6, yu_4, yu_5$. \\
\noindent\textbf{Canonical gadget rotations:} $\pi(x)=(u_1,y,u_6)$ and $\pi(y)=(u_4,x,u_5)$. \\
\noindent\textbf{Boundary order convention:} the trace boundary order for $C_2$ is normalized to $(u_1,u_4,u_5,u_6)$ up to reversal (Definition~\ref{def:C2-occ}). \\
\noindent\textbf{Terminal edits:} for each terminal $t\in\{u_1,u_6,u_4,u_5\}$ replace the deleted neighbor in its slot by the indicated gadget neighbor using \textsc{ReplaceNeighbor}.

\subsection{Refined \texorpdfstring{$C_4$}{C4} --- \texorpdfstring{\RefinedCFourGadgetVertices}{2}-vertex gadget}
\label{app:rot-refinedc4}

\noindent\textbf{Vertices added:} $x,y$. \\
\noindent\textbf{Edges added:} $xy, xu_1, xu_3, yu_2, yu_4$. \\
\noindent\textbf{Canonical gadget rotations:} $\pi(x)=(u_1,y,u_3)$ and $\pi(y)=(u_2,x,u_4)$. \\
\noindent\textbf{Terminal edits:} for each $u_i$, replace $v_i$ in its slot by $x$ (if $i\in\{1,3\}$) or $y$ (if $i\in\{2,4\}$).

\subsection{\texorpdfstring{$C_P$}{CP} (\texorpdfstring{$=\mathrm{pinch}(ii)$}{=pinch(ii)}) --- \texorpdfstring{\PinchGadgetVertices}{2}-vertex gadget with flip bit}
\label{app:rot-pinch}

\noindent\textbf{Vertices added:} $x,y$. \\
\noindent\textbf{Edges added:} $xy, xr, xs, yu_2, yu_4$. \\
\noindent\textbf{Canonical gadget rotations:}
\[
\pi(x)=(r,y,s),
\qquad
\pi(y)=
\begin{cases}
(u_2,x,u_4) & \varepsilon=0,\\
(u_4,x,u_2) & \varepsilon=1.
\end{cases}
\]
\noindent\textbf{Terminal edits:} replace $t$ by $x$ in the slots of $r$ and $s$, and replace $v_2$ and $v_4$ by $y$ in the slots of $u_2$ and $u_4$, respectively.
\section{3-connectivity locality for admissibility}
\label{app:3conn-preservation}

In the implementation, the replay checker verifies $3$-connectivity \emph{globally} after every reduction step as part of admissibility (see \S\ref{subsec:checker-semantics}).
For the \emph{finite} local-type completeness computation (Theorem~\ref{thm:certified-completeness}), we additionally need that admissibility of a fixed certified rule application can be decided from a bounded-radius closure around the modified patch.
This is formalized by the separator-locality lemma below.

\begin{lemma}[Separator locality for boundary-faithful disk replacements]
\label{lem:separator-locality}
Let $G$ be a plane graph and let $G'$ be obtained from $G$ by replacing a connected subgraph $P\subseteq G$ contained in a disk by another connected gadget $P'$, in such a way that:
(i) $V(G)\setminus V(P)=V(G')\setminus V(P')$ and $E(G)$ and $E(G')$ agree on this common vertex set, and
(ii) every edge with exactly one endpoint in $V(P)$ (respectively $V(P')$) is incident with a designated boundary set $B\subseteq V(G)\setminus V(P)$, and the boundary vertices $B$ are the same in $G$ and $G'$.

Let $S := B \cup N_{G'}(B)\cup N_{G'}^2(B)$ (distance measured in $G'$), and let $H' := G'[S]$ be the induced subgraph on $S$.
If $G'$ has a $2$-vertex separator, then $H'$ has a $2$-vertex separator.
In particular, if $H'$ is $3$-connected then $G'$ is $3$-connected.
\end{lemma}

\begin{proof}
Let $\{a,b\}$ be a $2$-vertex separator of $G'$, and let $C$ be a component of $G'-\{a,b\}$ which intersects the gadget vertex set $V(P')$.
If $\{a,b\}\cap S\neq\emptyset$ then, since $S$ is connected and contains $B$ as well as all vertices within distance at most $2$ from $B$, the disconnection witnessed by $C$ is already visible in $H'-\{a,b\}$, so $H'$ has a $2$-separator.

It remains to rule out the case $\{a,b\}\cap S=\emptyset$.
Under this assumption, the set $S$ (hence also $B$) lies entirely in the component of $G'-\{a,b\}$ that contains $V(P')$, because every vertex of $S$ is at distance at most $2$ from $B$ and $B$ attaches to $P'$ by construction.
Consequently, every component of $G'-\{a,b\}$ that is \emph{disjoint} from $S$ is also disjoint from $V(P')$ and from $B$.

Now observe that $G$ and $G'$ coincide on $V(G)\setminus V(P)=V(G')\setminus V(P')$, and every edge between vertices outside the patch is identical in $G$ and in $G'$.
Therefore, any vertex set $X$ disjoint from $V(P')$ has the same adjacency to the outside world in $G'$ as it does in $G$.
In particular, if $D$ is a component of $G'-\{a,b\}$ disjoint from $S$, then $D$ is also a component of $G-\{a,b\}$:
there is no edge from $D$ to $V(P)$ in $G$ (all such edges would have to go through $B\subseteq S$), and there is no new edge outside the patch in $G'$.
Hence $\{a,b\}$ also separates $G$, i.e.\ it is a $2$-vertex separator of $G$.

Applied to certified reductions in $\mathcal{Q}$, $G$ is $3$-connected, so such a separator cannot exist. Thus any $2$-separator of $G'$ must intersect $S$, and the separation is witnessed inside $H'$ as claimed.
\end{proof}

\noindent
For our certified reductions, the boundary set $B$ is contained in the radius-$3$ neighborhood of the reduced facial $4$-cycle (equivalently, of the inserted $2$-vertex gadget), so the induced subgraph $H'$ is precisely the \emph{radius-$3$ closure} used in the local-type enumeration for Theorem~\ref{thm:certified-completeness}.
...
The following commands enable full reproduction of the results presented in this paper. Scripts are in \texttt{src/} and tests in \texttt{tests/}.

\begin{itemize}
    \item \textbf{Logic Verification:} \verb|make verify-logic| \\
    Runs \verb|python src/verify_gadgets.py| to exhaustively verify lifting tables and interface completeness.
    \item \textbf{Base Case Enumeration:} \verb|make verify-base| \\
    Runs \verb|python tests/verify_base_cases.py| to enumerate and solve all graphs in $\mathcal{Q}$ up to $N_{\mathrm{base}}=14$.
    \item \textbf{Local Type Enumeration / Completeness:} \verb|make verify-local| \\
    Runs \verb|python src/enumerate_local_types.py| and \verb|python tests/verify_completeness.py| to generate local types and confirm admissible-step completeness (Theorem~\ref{thm:certified-completeness}) by exhaustive enumeration of rooted local types (Definition~\ref{def:local-type}) and production of at least one admissible witness per type.
    \item \textbf{Trace Verification:} \verb|make verify-trace| \\
    Runs \verb|python src/check_trace.py <tracefile>| to replay a certificate trace and verify all embedded invariants step-by-step, then lifts a base-case Hamiltonian cycle back to the original instance.
\end{itemize}

\subsection{Trusted Computing Base for Theorem~\ref{thm:certified-completeness}}
\label{app:tcb-thm41}

Theorem~\ref{thm:certified-completeness} is established by a finite, replayable computation (local-type enumeration plus admissibility witness production).  Since this is a computer-assisted completeness bridge, the paper must state exactly what code and primitives are \emph{trusted} to avoid circular ``it passed on my machine'' reasoning.

We follow a ``trust the checker'' philosophy: any search procedure may be heuristic and thus \emph{untrusted}, but a claimed witness is accepted only if it passes an explicit verifier that checks the $\mathcal{Q}$-invariants and the Lemma~\ref{lem:separator-locality} admissibility condition using the bounded neighborhood set $S = B \cup N(B) \cup N^2(B)$.

For Theorem~\ref{thm:certified-completeness}, the trusted computing base (TCB) consists of:

\begin{enumerate}
  \item \textbf{Platform.} \texttt{python3} and its standard library (\texttt{json}, \texttt{hashlib}, deterministic integer arithmetic).
  \item \textbf{Local-type enumeration domain.} \verb|src/enumerate_local_types.py|, including its rooted radius-3 type canonicalization and its planarity/embedding plumbing (via \texttt{networkx}).
  This component is in the TCB because omitting a realizable rooted local type would invalidate completeness.
  \item \textbf{Admissibility witness production.} \verb|src/produce_admissibility_witnesses.py|, specifically:
    (i) construction of the Lemma~\ref{lem:separator-locality} closure set $S$ for each candidate reduction, and
    (ii) the naive separator scan on $S$ that certifies the post-reduction graph remains 3-connected (hence in $\mathcal{Q}$) when combined with the other invariant checks.
  \item \textbf{Replay verifier for witnesses.} The invariant-checking routines invoked by the Theorem~\ref{thm:certified-completeness} verifier, centralized in \verb|src/barnette_proof.py| (simplicity, cubicity, bipartiteness, planarity using the stored rotation/face data, and 3-connectivity), together with their use in the completeness scripts.
  \item \textbf{Artifact integrity.} \verb|get_hashes_final.py| (SHA-256 over the published artifact files) and the fixed hash list in Appendix~\ref{app:hashes}.
\end{enumerate}


\smallskip
\noindent\textbf{For Theorem~\ref{thm:base-verified} (base cases).} The trusted computing base additionally includes:
\begin{enumerate}
  \item \textbf{Base-case enumeration toolchain.} The scripts (and any external tools they invoke) that generate the complete
        list of isomorphism types in $\mathcal{Q}$ with $|V|\le N_{\mathrm{base}}$ and write \verb|basecases.jsonl|.
        This component is in the TCB because omitting a realizable base case would invalidate the base step.
  \item \textbf{Cycle certificates and replay checker.} The code that (i) produces an explicit Hamilton cycle for each base case
        and (ii) verifies that the listed cycle visits every vertex exactly once.
\end{enumerate}
All other code (candidate generation, priority rules, and any speed-ups) is treated as \emph{untrusted}: it may fail to find a witness quickly, but it cannot create a false positive because witnesses are accepted only after replay verification.

\subsection{Expected Outputs and Audit Hashes}
\label{app:hashes}
The following results and hashes are associated with the verified commit \texttt{7ef267f675136003180a3877a5615bf5e047c1ee}. All hashes are SHA-256.

\paragraph{Primary Result ($N_{\max}=14$):}
\begin{itemize}
    \item \textbf{Counts:} 7,400 local types ($N=32$), 14 base cases.
    \item \verb|local_types.jsonl|: \texttt{6e21b7b2e67bdf02fc3f155934222282f08acddc59ad6abe5e2adf4c5102ddd2}
    \item \verb|extendible_witnesses.jsonl|: \texttt{ef71b962471eb90cacd49edbd360ed02d0ba39cf9a049d1a2170413a9097ee74}
    \item \verb|obstruction_witnesses.jsonl|: \texttt{d95c30a8340655548f42b8f1ca474d86f5e4deb43013b692afb23227d9f8b418}
    \item \verb|basecases.jsonl|: \texttt{294676c544b69848cdeed56e8ea67b4e0d1da68f42cae29cb08429e3820acb02}
    \item \verb|lift_library.json|: \texttt{4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945}
\end{itemize}

\paragraph{Sanity Check ($N_{\max}=12$):}
\begin{itemize}
    \item \textbf{Counts:} 7 local types, 3 extendible witnesses, 0 obstructions.
\end{itemize}

\paragraph{Reproducibility Notes:}
Counts may change across commits as the local-type generator evolves (e.g., historical runs at $n=12$ yielded 0 extendible types without horizontal edge support). The audit trail is established by the hashes recorded per commit.

\paragraph{Verification Environment:}
\begin{itemize}
    \item \textbf{Python:} 3.13.1
    \item \textbf{OS:} Microsoft Windows 10.0.26200
\end{itemize}

\section{Reproducibility}
The computational artifacts accompanying this paper allow full independent verification of all certified reductions and lifting steps. The base threshold is fixed at $N_{\mathrm{base}}=14$, with the cube graph $Q_3$ and the hexagonal prism $C_6\square K_2$ serving as the only terminal base cases.

\emph{Additional demonstration instances.} The implementation additionally demonstrates lifting on larger explicit examples (e.g., the octagonal prism $C_8\square K_2$). These runs are provided solely as sanity checks of the lifting and verification pipeline and play no role in the termination or correctness arguments of the proof.
