...
The reductions satisfy $\Delta n(C_2) = \CtwoDelta$, $\Delta n(\text{Refined } C_4) = \RefinedCfourDelta$, and $\Delta n(C_{\mathrm{pinch}(ii)}) = \PinchIIGadgetDelta$.
...
\begin{enumerate}
  ...
  \item \textbf{Measure decrease.} At minimum $|V|$ strictly decreases
        (by $|\RefinedCfourDelta|$ for Refined $C_4$ and by $|\PinchIIGadgetDelta|$ for $C_2$ and Pinch(ii)).
\end{enumerate}
...
%--------------------------------------------------------------------
\subsection{3-connectivity preservation via component-aware cut lifting}
\label{subsec:3conn-preservation}

We use a component-aware lifting lemma. The only hypothesis needed from the embedding framework
is that the patch meets the rest of the graph exactly in the terminal set $T$
(and that no other edges cross the patch boundary).

\begin{definition}[Terminal-side separation inside a patch]
Let $P$ be a graph containing the terminal set $T$ and let $S\subseteq V(P)$.
For disjoint nonempty sets $A,B\subseteq T$, we say that $S$ \emph{separates $A$ from $B$ in $P$}
if there is no $A$--$B$ path in $P-S$.
\end{definition}

\begin{lemma}[Component-aware 2-cut lifting template]
\label{lem:cut-lifting-template}
Let $G$ be obtained by gluing an \emph{outside graph} $G_{\mathrm{out}}$ to a \emph{patch} $P_H$
along a common terminal set $T$ (and with no other shared vertices/edges).
Let $G'$ be obtained from the same $G_{\mathrm{out}}$ by replacing $P_H$ with another patch $P_K$
on the same terminal set $T$.

Let $S\subseteq V(G')$ with $|S|\le 2$, and write
$S_{\mathrm{out}}=S\cap V(G_{\mathrm{out}})$ and $S_K=S\cap V(P_K)$.
Assume that $G'-S$ is disconnected, and let $A',B'$ be distinct components of $G'-S$.
Define terminal sides
\[
T_A = T\cap V(A'),\qquad T_B = T\cap V(B').
\]

If there exists a set $S_H\subseteq V(P_H)$ with $|S_H|\le |S_K|$ that separates $T_A$ from $T_B$
in $P_H$, then the set
\[
S^* := S_{\mathrm{out}} \cup S_H
\]
disconnects $G$, and $|S^*|\le |S|\le 2$.
In particular, if $G$ is $3$-connected, then no such $S$ exists and hence $G'$ is $3$-connected.
\end{lemma}

\begin{proof}
Let $S$ and $A',B'$ be as stated. Consider any path $Q$ in $G-S^*$ from a vertex of $A'$ to a vertex
of $B'$. Since $G_{\mathrm{out}}$ is identical in $G$ and $G'$ and we removed the same set
$S_{\mathrm{out}}$, the only way for $Q$ to cross between the two terminal sides is through the patch.
But $S_H$ separates $T_A$ from $T_B$ in $P_H$, so $Q$ cannot cross through $P_H$.
Therefore no such path $Q$ exists, so $G-S^*$ is disconnected. The size bound is immediate.
\end{proof}

\begin{lemma}[Robustness of the modified $C_2$ reduction]
\label{lem:robust-c2}
The modified $C_2$ gadget described in Section~\ref{subsec:red-c2} satisfies the hypothesis of
Lemma~\ref{lem:cut-lifting-template} for all cuts $S$ with $|S|\le 2$.
Consequently, if $G$ is $3$-connected then the reduced graph $G'$ is $3$-connected.
\end{lemma}
...
\begin{lemma}\label{lem:3conn-C4}
\prereq{Fact~\ref{fact:projection}, 3-connectivity definition}
Let $G\in\mathcal Q$ contain a certified refined $C_4$ configuration with internal
vertices $\{v_1,v_2,v_3,v_4\}$ and terminals $\{u_1,u_2,u_3,u_4\}$,
and let $G'$ be obtained by the refined $C_4$ reduction (\RefinedCFourGadgetVertices-vertex gadget).
Then $G'$ is 3-connected.
\end{lemma}

\begin{proof}
The gadget is $K=\{x,y\}$ with terminal edges $xu_1, xu_3, yu_2, yu_4$ and internal edge $xy$ (Definition~\ref{def:red-c4}).
Assume for contradiction that $G'$ has a vertex cut $S$ with $|S|\le2$.

Define $S_{\text{out}}:=S\cap\bigl(V(G')\setminus K\bigr)$. We construct a cut $S^*$ in $G$ with $|S^*| \le 2$.
For each gadget vertex in $S$, select one of its original attachment vertices:
\begin{itemize}
    \item If $x \in S$, add $v_1$ to $S^*$.
    \item If $y \in S$, add $v_2$ to $S^*$.
\end{itemize}
Then $|S^*| = |S_{\text{out}}| + |S \cap \{x,y\}| = |S| \le 2$.
By Fact~\ref{fact:projection} and the connectivity of the original patch $H$ (every terminal $u_i$ has a path to every other terminal in $H \setminus S^*$, possibly requiring a case analysis on $v_3, v_4$ survival), the existence of a cut $S$ in $G'$ implies a cut $S^*$ in $G$.
Since $G$ is 3-connected, no such $S^*$ exists, hence no such $S$ exists. Therefore $G'$ is 3-connected.
\end{proof}

%--------------------------------------------------------------------
\subsection*{Lemma 6.4.3  ( $C_{\text{pinch}(ii)}$ reduction preserves 3-connectivity )}

\begin{lemma}\label{lem:3conn-pinch}
\prereq{Fact~\ref{fact:projection}, 3-connectivity definition}
Let $G\in\mathcal Q$ contain a certified $C_{\text{pinch}(ii)}$ configuration with
deleted vertices $\{v_1,v_2,v_3,v_4,w,t\}$ and terminals $T=\{r,s,u_2,u_4\}$
(boundary edges $rt, st, u_2v_2, u_4v_4$).  
Let $G'$ be obtained by the $C_{\text{pinch}(ii)}$ reduction: delete those vertices
and add \PinchGadgetVertices-vertex gadget vertices $x,y$ with edges $xr,\;xs,\;xy,\;yu_2,\;yu_4$.
Then $G'$ is 3-connected.
\end{lemma}

\begin{proof}
Assume for contradiction that $G'$ has a vertex cut $S$ with $|S|\le2$;
choose $S$ inclusion-minimal.

Define
\[
S_{\text{out}} \;:=\; S\cap\bigl(V(G')\setminus\{x,y\}\bigr),
\]
and set
\[
S^* \;:=\; S_{\text{out}}\;\cup\;
\bigl(\{t\}\text{ if }x\in S\bigr)\;\cup\;
\bigl(\{v_2,v_4\}\text{ if }y\in S\bigr).
\]

\noindent
\textbf{Step 1: $|S^*|\le 2$.}
Again we examine $S\cap\{x,y\}$.

\begin{itemize}
\item \emph{Case 1: $S\cap\{x,y\}=\emptyset$.}  
Then $S^*=S$, so $|S^*|\le2$.

\item \emph{Case 2: $x\in S$, $y\notin S$.}  
Then $S^* = S_{\text{out}}\cup\{t\}$.  Since $|S|\le2$ and $x\in S$,
$|S_{\text{out}}|\le1$, hence $|S^*|\le2$.

\item \emph{Case 3: $y\in S$, $x\notin S$.}  
Then $S^* = S_{\text{out}}\cup\{v_2,v_4\}$.  
If $S_{\text{out}}\neq\emptyset$, then $|S^*|\ge3$, impossible.
The same minimality argument as in Lemma~\ref{lem:robust-c2} (Case~2) shows that
$S_{\text{out}}$ must be empty; otherwise $y$ alone would already be a cut.
Thus $S=\{y\}$ and $|S^*|=2$.

\item \emph{Case 4: $\{x,y\}\subseteq S$.}  
Then $S=\{x,y\}$.  We show this cannot be a minimal cut.
If $G'-\{x\}$ were disconnected, then $\{x\}$ would be a cut, contradicting minimality.
Hence $G'-\{x\}$ is connected.
Now remove $y$ from $G'-\{x\}$.  In $G'-\{x\}$, $y$ is adjacent only to $u_2$ and $u_4$.
Because $G'-\{x\}$ is connected, $u_2$ and $u_4$ are connected to the rest of the graph
(including $r$ and $s$) via paths avoiding $y$.  Therefore $G'-\{x,y\}$ is connected,
contradicting that $S$ is a cut.  Hence $S$ cannot contain both $x$ and $y$.
\end{itemize}
Thus $|S^*|\le2$ in all cases.

\noindent
\textbf{Step 2: The Projection Fact applies.}
We have $g(r)=g(s)=x$, $h(r)=h(s)=t$, and $g(u_2)=g(u_4)=y$, $h(u_2)=v_2$, $h(u_4)=v_4$.
By construction, if $x\in S$ then $t\in S^*$; if $y\in S$ then $\{v_2,v_4\}\subseteq S^*$.
Condition (2) of Fact~\ref{fact:projection} holds for every terminal; condition (1) holds by
definition of $S_{\text{out}}$.

\noindent
\textbf{Step 3: Contradiction with $G$ being 3-connected.}
Since $G'-S$ is disconnected, Fact~\ref{fact:projection} implies $G-S^*$ is disconnected,
but $|S^*|\le2$, contradicting the 3-connectivity of $G$.

Therefore $G'$ is 3-connected.
\end{proof}

\bigskip
These three lemmas together establish that each certified reduction preserves
3-connectivity, fulfilling requirement (CRF3) of the Certified Reduction Framework.
...
\begin{proof}
One verifies by inspection that each path in Table~\ref{tab:lifting-c4} connects the correct terminals and covers $V(H)$ exactly once. The result follows from Lemma~\ref{lem:table-correctness}. (Parallel interfaces cannot occur: bipartite parity forbids them.)
\end{proof}
...
\begin{proof}
Consider the boundary edges $B = \{u_1v_1, u_2v_2, u_3v_3, u_4v_4\}$. Since $H$ is a cycle, $|H \cap B|$ is even. It cannot be 0 (would disconnect $H$). As shown in Table~\ref{tab:lifting-c4}, the only bipartite-consistent pairings that cover all vertices of $Q$ are the four cross paths connecting $\{u_1,u_3\}$ to $\{u_2,u_4\}$. Two-path interfaces would require pairing same-parity terminals, which is impossible on a facial 4-cycle in a bipartite graph while covering all vertices.
\end{proof}
...
This value is chosen to accommodate the largest vertex-count decrease among our certified reductions:
$\Delta n(C_2) = \CtwoDelta$, $\Delta n(\text{Refined } C_4) = \RefinedCfourDelta$, and $\Delta n(\mathrm{Pinch}(ii)) = \PinchIIGadgetDelta$ (see Definitions~\ref{def:red-c2}--\ref{def:red-pinch} for the explicit vertex changes).
In particular, for any $G\in\mathcal Q$ with $|V(G)|\ge 16$, every certified reduction produces a graph
$G'\in\mathcal Q$ with $|V(G')|\ge 12$, while graphs with $|V|\le 14$ are handled directly by the base solver.
\end{definition}
...
in which the diagonal-isolation condition is dropped, justified by the robustness
of the \RefinedCFourGadgetVertices-vertex gadget and its routing-table 3-connectivity proof (Section~\ref{subsec:3conn-preservation}).
...
\begin{proof}
Each reduction strictly decreases the vertex count $n(G)$ by at least 2:
\begin{itemize}
    \item \textbf{$C_2$ reduction:} $n(G') = n(G) + \CtwoDelta$.
    \item \textbf{$C_{\text{pinch}(ii)}$ reduction:} $n(G') = n(G) + \PinchIIGadgetDelta$.
    \item \textbf{Refined $C_4$ reduction:} $n(G') = n(G) + \RefinedCfourDelta$.
\end{itemize}
...
\label{thm:linear}
Assuming each reduction decreases $n$ by at least $2$ (and by at most a constant),
...
$|V|-|E|+F=2$.
\item \textbf{(Optional strict mode) 3-connectivity.} If \texttt{--strict} is enabled, the checker runs a global
3-vertex-connectivity test (e.g.\ SPQR-based triconnectivity) on $G$ and rejects otherwise.
\end{enumerate}
...
\]
where $\Delta n(\textsf{C2})=2$, $\Delta n(\textsf{refinedC4})=2$, and $\Delta n(\textsf{pinch(ii)})=4$ (per your certified reductions section).
