...
pattern = r'The structural requirements for a certified reduction defined in Section~\\ref\{sec:catalog\} entail additional side-conditions \(e\.g\., distinctness of all extended neighborhood vertices\)\. The following completeness lemma bridges this topological guarantee to the fully certified occurrences needed for the algorithm\.\s*\\begin\{lemma\}\[Certified Completeness\]\s*\\label\{lemma:certified_completeness\}\s*If \$G \\in \\mathcal\{Q\}\$ contains a topological occurrence of \$C_2\$, \$C_4\$, or \$C_P\$ as defined in Theorem~\\ref\{thm:completeness\}, then \$G\$ must contain a strict certified occurrence of \$C_2\$, refined \$C_4\$, or \$C_P\$ \(meeting all side-conditions and vertex distinctness requirements defined in Section~\\ref\{sec:catalog\}\)\.\s*\\end\{lemma\}\s*\\begin\{proof\}\s*TODO: prove\. \(Exploration of intersecting labels and overlapping neighborhood cases\)\.\s*\\end\{proof\}'

if re.search(pattern, text):
    text = re.sub(pattern, new_block.lstrip('\n').replace('\\', '\\\\'), text)
    ...
    print("FAILED to find old block with regex.")
