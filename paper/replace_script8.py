...
There exists a finite constant $N_{base}$ (e.g., $N_{base} = 64$) such that every graph $G \in \mathcal{Q}$ with $|V(G)| \le N_{base}$ contains a Hamiltonian cycle. (This finite set has been exhaustively verified; see Section \ref{sec:verification}).
\end{fact}
...
if r'\section{Algorithm and Implementation}' in text:
    text = text.replace(r'\section{Algorithm and Implementation}', new_block.lstrip('\n'))
    ...
    with open(r'c:\Users\charl\Documents\New Math Problem\paper\LaTeX_Pure_Maths_revised.txt', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Success Regex Inject Algorithm Implementation")
else:
