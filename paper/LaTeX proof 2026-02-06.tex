...
The local-to-global justification for the $3$-connectivity component is Lemma~\ref{lem:separator-locality} in Appendix~\ref{app:3conn-preservation}.
...
In the verified implementation, the finite library is materialized for audit as \texttt{artifacts/lift_library.json} (exported from the internal cache used by the lifting routine).
\end{proof}
...
The unavoidability theorem (Theorem~\ref{thm:completeness}) guarantees that \emph{some} topological occurrence of one of the configurations
$\mathcal{C}_2$, $\mathcal{C}_4$, or $\mathcal{C}_{\mathrm{pinch(ii)}}$ exists around a facial $4$-cycle in every state $(G,\pi)\in\mathcal{Q}$.
For the inductive proof we require a \emph{certified} occurrence that can be reduced while preserving membership in $\mathcal{Q}$ and enabling a
Hamilton cycle to be lifted.
...
\begin{proof}
Part (1) is immediate from Definition~\ref{def:local-type}.

For (2), the script \texttt{src/enumerate\_local\_types.py} enumerates the finite set $\mathcal{T}$ of rooted
radius-$3$ local types compatible with the defining constraints of $\mathcal{Q}$
(cubic, bipartite, planar rotation system, and a rooted facial $4$-cycle).
The script \texttt{tests/verify\_completeness.py} then searches each enumerated type for a certified occurrence of
one of the catalog configurations and a witness certificate whose replayed reduction is accepted by the independent checker
(\S\ref{subsec:checker-semantics}). The resulting witness logs (and audit hashes) are recorded in Appendix~\ref{app:hashes}.

Now let $(G,\pi)\in\mathcal{Q}$ with a facial $4$-cycle $Q$ (exists by Lemma~\ref{lem:existence-f4}) and let $\tau\in\mathcal{T}$
be the rooted local type of $(G,\pi)$ at $Q$.
By construction, the witness certificate produced for $\tau$ specifies a reduction step whose footprint lies within
graph distance at most $2$ from $V(Q)$, hence within the radius-$3$ closure graph underlying $\tau$.
All checker postconditions except $3$-connectivity are purely local and therefore depend only on the patch and its boundary attachments.
For $3$-connectivity, Lemma~\ref{lem:separator-locality} shows that if the replayed post-state of the radius-$3$ closure is $3$-connected,
then the corresponding ambient post-state is also $3$-connected.
Therefore the same reduction step is admissible in the full state $(G,\pi)$.
\end{proof}



\paragraph{Remark (role in Paper~1 vs.\ Paper~2).}
For Paper~1 (computer-assisted but fully rigorous), Theorem~\ref{thm:certified-completeness} is part of the trusted proof pipeline:
it is a finite, replay-checkable claim about bounded local neighborhoods.
In Paper~2 (purely mathematical), one would replace this step by a fully analytic admissibility argument (and in particular a non-computational
$3$-connectivity preservation proof).

\subsection{Computational base verification}

\begin{theorem}[Base-case verification (computational, exhaustive)]
\label{thm:base-verified}
Fix $N_{\mathrm{base}}$ as in Definition~\ref{def:Nbase}. For every $G\in\mathcal{Q}$ with $|V(G)|\le N_{\mathrm{base}}$,
the script \texttt{tests/verify\_base\_cases.py} exhaustively enumerates all isomorphism types of plane graphs in $\mathcal{Q}$ with $|V|\le N_{\mathrm{base}}$
(using the trusted external generator \texttt{plantri} and filtering by checker-validated predicates), and for each instance produces an explicit Hamiltonian cycle witness.
The complete list of instances and cycle witnesses is written to \texttt{artifacts/basecases.jsonl}.
\end{theorem}

\begin{proof}
This is established by exhaustive computation; see Appendix~\ref{app:reproduction}.

The base-case generator used by \texttt{tests/verify\_base\_cases.py} is complete up to plane isomorphism:
it enumerates all candidate plane graphs via a trusted external enumerator (\texttt{plantri}). The wrapper \texttt{src/plantri\_wrapper.py} parses \texttt{plantri -a} output into rotation-system objects used throughout the checker. It then filters them using
the independent replay checker predicates (simplicity, cubicity, bipartiteness, planarity-from-rotation, and $3$-connectivity).
In particular, completeness of the \emph{generation} step relies on \texttt{plantri}'s enumeration correctness,
while all membership tests for $\mathcal{Q}$ and all Hamiltonicity witnesses are validated independently by our checker.

For each generated base instance, the script computes an explicit Hamiltonian cycle (using a small exact routine suitable at this size),
and the checker validates that the reported cycle is a simple cycle visiting every vertex exactly once.
The cycle certificates and the complete list of generated instances are recorded in \texttt{artifacts/basecases.jsonl}; the corresponding SHA-256 checksums are listed in \texttt{hashes_output.txt} (Appendix~\ref{app:hashes}).
\end{proof}
...
\section{3-connectivity locality for patch reductions}\label{app:3conn-preservation}

In Paper~1, $3$-connectivity is enforced by the replay checker as part of admissibility
(\S\ref{subsec:checker-semantics}).  However, Theorem~\ref{thm:certified-completeness}
is proved by enumerating isolated radius-$3$ rooted local types and exhibiting, for each type,
at least one reduction step whose \emph{local} post-state passes the checker predicates.
The reviewer correctly observes that $3$-connectivity is global, so we must justify that a
loss of $3$-connectivity caused by applying a certified patch reduction is already witnessed
within the radius-$3$ closure used in the local-type enumeration.

\begin{lemma}[Separator locality for radius-$3$ closure]\label{lem:separator-locality}
Let $(G,\pi)\in\mathcal{Q}$ and let $Q$ be a facial $4$-cycle of $G$.
Let $H$ be the induced embedded subgraph on all vertices at graph distance at most $3$ from $V(Q)$
(i.e.\ the underlying graph of the rooted local type in Definition~\ref{def:local-type}).
Let $\mathrm{cert}$ be a certified occurrence whose footprint is contained in the vertices of $H$ at distance at most $2$ from $V(Q)$,
and let $(G',\pi'):=\textsc{Reduce}(G,\pi;\mathrm{cert})$ be the ambient post-state.
Let $(H',\pi_H')$ be the post-state obtained by replaying the same reduction on $(H,\pi_H)$ under the checker semantics.
If $H'$ is $3$-connected, then $G'$ is $3$-connected.
\end{lemma}

\begin{proof}
Suppose for contradiction that $G'$ is not $3$-connected, and let $\{a,b\}$ be a $2$-vertex separator in $G'$.
Let $P$ denote the vertex set modified by the reduction (deleted patch vertices together with any newly inserted gadget vertices).
By construction, $P$ is contained in the distance-$2$ neighborhood of $V(Q)$, hence $P\subseteq V(H)$, and
$G'$ and $G$ are identical on $V(G)\setminus P$.

\emph{Claim 1:} At least one of $a,b$ lies in $V(H)$.
Indeed, if $\{a,b\}\cap V(H)=\emptyset$, then the induced subgraph on $V(G)\setminus P$ is unchanged by the reduction,
so any separation of vertices outside $H$ witnessed in $G'-\{a,b\}$ is also witnessed in $G-\{a,b\}$, contradicting that $G$ is $3$-connected.

\emph{Claim 2:} In fact $\{a,b\}\subseteq V(H)$.
Assume w.l.o.g.\ that $a\in V(H)$ and $b\notin V(H)$.
Let $C$ be a component of $G'-\{a,b\}$ that is disjoint from $V(H)$ (if none exists, then $\{a,b\}$ separates only vertices of $H$ and we are done).
Every vertex of $C$ has all its incident edges outside $P$, hence the same component $C$ exists in $G-\{a,b\}$.
Since $G$ is $3$-connected, there exist three internally vertex-disjoint paths in $G$ from any vertex of $C$ to $V(Q)$.
At most one of these paths can use $b$ (as an internal vertex), and none can use vertices of $P$ except possibly at their final entry into $H$.
Thus there remains a path from $C$ to $V(Q)$ in $G-\{a,b\}$ that avoids $P$ entirely; because the reduction does not modify edges outside $P$,
the same path exists in $G'-\{a,b\}$, contradicting that $C$ is separated in $G'-\{a,b\}$.
Therefore no such component $C$ exists, and $\{a,b\}\subseteq V(H)$.

Now, because the reduction and the checker replay semantics agree on the induced subgraph $H$ (both are purely local patch replacements with the same
boundary attachments and rotation updates), the same pair $\{a,b\}$ separates $H'$.
This contradicts that $H'$ is $3$-connected.
\end{proof}

Lemma~\ref{lem:separator-locality} is the only place where we appeal to a handwritten argument about $3$-connectivity.
All other invariants (simplicity, cubicity, bipartiteness, and planarity from the rotation system) are checked directly by the replay checker.
...
\begin{itemize}
    ...
    Runs \verb|python tests/verify_base_cases.py| to enumerate and solve all graphs in $\mathcal{Q}$ up to $N_{\mathrm{base}}=14$, writing cycle witnesses to \verb|artifacts/basecases.jsonl| and exporting the lifting-cache snapshot to \verb|artifacts/lift_library.json|.
    \item \textbf{Local Type Enumeration / Completeness:} \verb|make verify-local| \\
    Runs \verb|python src/enumerate_local_types.py| and \verb|python tests/verify_completeness.py| to generate local types and confirm admissible-step completeness (Theorem~
ef{thm:certified-completeness}) by exhaustive enumeration of rooted local types (Definition~
ef{def:local-type}) and production of at least one admissible witness per type.
...
The following results and hashes are from the accompanying artifact manifest (
erb|hashes_output.txt|). All hashes are SHA-256.


oindent The lifting subsystem maintains a persistent in-process cache (keyed by canonical embedded-graph hashes) so that repeated patch searches reuse previously verified lift witnesses; the exported 
erb|artifacts/lift_library.json| is a serialized snapshot of this cache.


\paragraph{Primary Result ($N_{\max}=14$):}
\begin{itemize}
    \item \textbf{Counts:} 77 local types, 11 extendible witnesses, 0 obstructions.
    \item \verb|artifacts/local_types.jsonl|: \texttt{33ddafc1b4e2ade35bc399bab78509069de68564f91e1ce7afd7d90ec249aa3c}
    \item \verb|artifacts/extendible_witnesses.jsonl|: \texttt{8ca386973b1060453a877b732420f5ebf1401a95e0b69eeef9766c4556ace064}
    \item \verb|artifacts/obstruction_witnesses.jsonl|: \texttt{e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855} (empty file; no obstructions found)
    \item \verb|artifacts/basecases.jsonl|: \texttt{294676c544b69848cdeed56e8ea67b4e0d1da68f42cae29cb08429e3820acb02}
    \item \verb|artifacts/lift_library.json|: \texttt{4f53cda18c2baa0c0354bb5f9a3ecbe5ed12ab4d8e11ba873c2f11161202b945}
\end{itemize}

\noindent The file \verb|hashes_output.txt| records the same SHA-256 digests as an audit manifest produced by \verb|python verification/get_hashes_final.py|.
...
The computational artifacts accompanying this paper allow full independent verification of all certified reductions and lifting steps. The base threshold is fixed at $N_{\mathrm{base}}=14$. The base-case artifact \texttt{artifacts/basecases.jsonl} exhaustively lists every $G\in\mathcal{Q}$ with $|V(G)|\le N_{\mathrm{base}}$ together with a checker-verified Hamiltonian cycle witness.
