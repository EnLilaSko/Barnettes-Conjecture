...
new_block = r"""The structural requirements for a certified reduction defined in Section~\ref{sec:catalog} entail additional side-conditions (e.g., distinctness of all extended neighborhood vertices). We now bridge this topological guarantee to fully certified, admissible reductions purely structurally.
...
match = re.search(r'(The structural requirements.*?\\end\{proof\})', text, re.DOTALL)
if match:
