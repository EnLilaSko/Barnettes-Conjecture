...
\label{cor:planar-C2}
For a certified $C_2$ reduction step, the deleted domino subgraph forms a disk patch whose boundary terminals are the four terminals in the boundary order recorded by the certificate trace.
...
Because every cubic bipartite graph has an even number of vertices, any $G\in\mathcal{Q}$ with $|V(G)|>N_{\mathrm{base}}$ satisfies $|V(G)|\ge N_{\mathrm{base}}+2=16$.
Since the maximum vertex-count decrease in one step is $4$ (for $C_2$ and $C_P$), any admissible reduction from such a graph yields
\[
|V(G')| \ge 16-4=12,
\]
which remains within the safe range handled by the recursive procedure and the verified base range.
\end{definition}
...
\begin{proof}
By Lemma~\ref{lem:existence-f4}, $G$ contains a facial $4$-cycle $Q$.
By Theorem~\ref{thm:completeness}, each facial $4$-cycle yields a topological occurrence of one of the catalog configurations.

A priori, a topological occurrence may fail local certification or may yield a non-admissible post-state (e.g.\ violating $3$-connectivity).
Lemma~\ref{lem:small-obstructions} bounds (and in the current verification run eliminates) such dead ends at size $\le 14$.
Therefore, for $|V(G)|>N_{\mathrm{base}}$, at least one facial $4$-cycle yields an admissible certified occurrence.
\end{proof}
...
In the current verified run, the local-type enumeration found \textbf{77} rooted radius-$3$ local types around a facial $4$-cycle, \textbf{11} extendible witness completions (including the cube at $n=8$), and \textbf{0} obstruction witnesses up to $N_{\max}=14$. See Appendix~\ref{app:reproduction} for reproducibility parameters and SHA-256 audit hashes.
\end{lemma}

\begin{proof}
This is established by a finite, reproducible computation.

The script \texttt{src/enumerate\_local\_types.py} enumerates rooted radius-$3$ local types around a distinguished facial $4$-cycle as subcubic bipartite partial graphs with stubs (missing degree slots), allowing horizontal edges between vertices in the same distance layer (required to include the cube neighborhood).
For each local type, the script exhaustively searches all completions to graphs $G \in \mathcal{Q}$ with $|V(G)| \le N_{\max}=14$.
For each completion, it runs \texttt{verify\_completeness}, which:
(i) generates all candidate topological occurrences from the $4$-face classification,
(ii) filters those passing the local certification predicates, and
(iii) for each locally certified candidate, constructs the reduced instance and checks admissibility (including planarity-from-rotation, cubicity, bipartiteness, and $3$-connectivity).
A local type is marked \emph{bad} only if \emph{no} completion admits any admissible move.

The artifacts are written to \texttt{artifacts/local\_types.jsonl}, \texttt{artifacts/extendible\_witnesses.jsonl}, and \texttt{artifacts/obstruction\_witnesses.jsonl}.
In the verified run corresponding to the commit listed in the data-availability section, the generator produced 77 local types and 11 extendible witnesses, and no obstruction witnesses were found up to $N_{\max}=14$.
Thus any obstruction to admissible reducibility (if it exists) must occur on at most $14$ vertices, and none exist in the verified range.
See Appendix~\ref{app:reproduction} for one-command reproduction.
\end{proof}
...
For $N_{\mathrm{base}}=14$, the enumerated isomorphism types in $\mathcal{Q}$ are:
\begin{itemize}
...
\EndIf
\State $C \gets \textsc{FindAdmissibleConfig}(G)$ \Comment{search certified candidates until the checker postcheck passes}
