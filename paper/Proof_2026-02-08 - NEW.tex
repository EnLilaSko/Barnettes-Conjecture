...
In the current verified run, the local-type enumeration found \textbf{77} rooted radius-$3$ local types around a facial $4$-cycle, \textbf{11} extendible witness completions (including the cube at $n=8$), and \textbf{0} obstruction witnesses up to $N_{\max}=14$.
\end{lemma}
...
\begin{itemize}
    \item \textbf{Base Cases:} \verb|artifacts/Q_upto_14.jsonl| contains the cube ($n=8$) and hexagonal prism ($n=12$). The Hamilton cycles returned by the solver are recorded in the base-case artifacts/logs.
    \item \textbf{Local Types:} \verb|artifacts/local_types.jsonl| contains 77 rooted local types in the verified run; \verb|artifacts/extendible_witnesses.jsonl| contains 11 extendible witnesses; \verb|artifacts/obstruction_witnesses.jsonl| is empty in the verified run (0 obstructions up to $N_{\max}=14$).
    \item \textbf{Auditability:} each command prints the current commit hash and emits a timestamped log.
\end{itemize}
