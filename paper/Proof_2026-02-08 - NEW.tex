...
\begin{enumerate}
  ...
  \item \textbf{Vertex-count delta.} The checker computes $\Delta n := |V(G')|-|V(G)|$
        ...
        and $\Delta n=\CtwoDelta$ for $C_2$ and $C_{\text{pinch}(ii)}$.
\end{enumerate}
...
$|V(G)|>N_{\mathrm{base}}$ contains an \emph{admissible} certified occurrence of $C_2$, $C_{\mathrm{pinch}(ii)}$, or refined $C_4$.
\end{theorem}
...
gives rise to an admissible certified occurrence of $C_2$, $C_{\mathrm{pinch}(ii)}$, or refined $C_4$.
\end{proof}


\begin{lemma}[Small obstructions to admissible reducibility are finite]
\label{lem:small-obstructions}
Let $\mathcal{R}=\{R_{C_2},R_{C_{\mathrm{pinch}(ii)}},R_{C_4}\}$ be the catalog.
...
the local $4$-face classification yields at least one \emph{admissible} certified occurrence of $C_2$, $C_{\text{pinch}(ii)}$,
or refined $C_4$.
\end{lemma}
\begin{proof}
...
compatible with cubicity and bipartiteness, and filters those extending to $3$-connected instances.
