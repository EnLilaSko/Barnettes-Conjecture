...
Fix $N_{\mathrm{base}}$ as in Definition~\ref{def:Nbase}.
For every $G \in \mathcal{Q}$ with $|V(G)| \le N_{\mathrm{base}}$, the script
\texttt{tests/verify\_base\_cases.py} enumerates all abstract graphs in $\mathcal{Q}$ with $|V| \le N_{\mathrm{base}}$ up to isomorphism (one representative embedding per graph) and runs an exact Hamiltonicity solver,
outputting either a Hamiltonian cycle or a certificate of nonexistence.
\end{theorem}

\begin{proof}
This is established by exhaustive computation; see Appendix~\ref{app:reproduction}.

For $N_{\mathrm{base}}=14$, the audited generator run described in Appendix~B produced the following
isomorphism types in $\mathcal{Q}$ with $n\le 14$:
\begin{itemize}
  \item $n=8$: the cube graph $Q_3$.
  \item $n=12$: the hexagonal prism $C_6\square K_2$.
\end{itemize}
In each generated instance the exact solver returns an explicit Hamilton cycle, and the checker verifies it;
the cycle certificates are recorded in the corresponding artifact file.

\smallskip
\noindent\textbf{Remark.}
The list above is a statement about the output of the generator used in our verification pipeline at the cited commit.
We do not claim it exhausts all abstract isomorphism types in $\mathcal{Q}$ on $n\le 14$ vertices; any additional base
instances would simply appear as additional lines in the artifact file and be handled identically by the exact solver
and checker.
...
The following results and hashes are associated with the verified commit \texttt{7ef267f}. All hashes are SHA-256.
